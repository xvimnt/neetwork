...
import { UIButtonsPagination } from "../UI/UIButtonsPagination";
import { type IFilter, UITabs } from "../UI/UITabs";
import { UITable } from "../UI/UITable";
import { SearchIcon } from "../UI/Icons";
import { UIDebouncer } from "../UI/UIDebouncer";

export interface ILayoutTableData {
  title: string;
  description?: string;
...
export const LayoutTableData = ({
  ...
  // Verify if it has filters
  const hasFilters = filters && filter && setFilter;
  // Verify if has search
  const hasSearch = search && setSearch;
  // Verify if has pagination
  const hasPagination = page && setPage && totalPages && handleNext;
      ...
      <div className="w-full  sm:flex sm:items-center sm:justify-between">
        <div>
          <div className="flex items-center gap-x-3">
            <h2 className="text !text-3xl font-bold">{title}</h2>
